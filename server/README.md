...
- The server run also shows Entry / SL / TP1–3 with a live mark price and unrealised P&L (from the public price, refreshed every 2s — an estimate that ignores fees, like the in-tab bot's) and a Close button (needs a verified key for that exchange/network in the browser you click it from; the server books the closure itself).
- If a session the page saw running disappears (server restart/redeploy, free-plan sleep, or stopped from another device), the switch goes OFF and says so in red instead of silently showing idle; a stop done from the page itself doesn't raise it. The page only polls while its tab is visible, so on a free Render instance nothing keeps the server awake when you're on another tab — see the sleep note above (`/api/health` is a public route an uptime pinger can hit; `WORKER_AUTOARM` restarts the env-configured exchange after a wake-up).
- **From another device:** opening the page only *reads* `/api/worker/status`; it never starts, stops or duplicates the server run. Enter the access token once on that device (an "Enter access token" button appears until you do) and the switch and cards show the same live numbers. Switching off from any device stops the one shared server session. API keys are per browser, so a device without a verified key can watch, stop and (no key needed) switch off — but not switch on or close positions.
