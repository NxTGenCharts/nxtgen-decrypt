...
// The excluded-pairs list lives in excludedSymbols.js. Re-exported
...
export function runScanCycle(cfg, dayState, opts){
  const symbols = (opts && opts.symbols) || TRADEABLE_FUTURES_SYMBOLS.filter(x => !EXCLUDED_FUTURES_SYMBOLS.has(x));
...
export function evaluateSymbol(symbol, snap, regime, cfg, dayState, btcShock, nowMs){
  ...
  // nothing else qualified, so the scanner says WHY instead of a blank
  // "no setup".
...
export function managePositions(dayState, tradeHistory, cfg){
  ...
  for(const pos of dayState.positions){
    ...
    if(hitSL){
      ...
      closeTrade(pos, pos.stop, pnl, 'STOP_LOSS', dayState, tradeHistory, undefined, 0);
