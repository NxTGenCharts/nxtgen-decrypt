...
**Backtest tab → "Run settings sweep" button (no Node needed)** — same sweep, in the browser: after any Quant backtest (use *Last 90 days*), the validation section has the button. It reuses that run's candles, tests ~22 settings on background Web Workers (`quant/sweep-runner.js`, `sweep-worker.js`; the config list and scoring are shared with the Node tool in `quant/sweep.js`), and shows TRAIN vs TEST with the profitable-on-both rows highlighted and a "Copy results as text" button. Also fixed: the walk-forward re-runs used a partial strategy map, so other default-enabled strategies could trade alongside Quant; every other strategy is now explicitly off.

**`tools/quant-sweep.mjs`** (optional, Node 22+) — runs the app's real backtest engine (same code as the Backtest tab) over ~23 configs (or `--grid full` = 96) on 90 days of real candles, downloaded through the Render proxy and cached, in parallel worker threads. It splits each config's trades chronologically into TRAIN (first ~2/3) and TEST (last ~1/3), ranks on TRAIN, and reports TEST beside it so overfit configs are visible. Needs Node 22+, the fixed `server.js` deployed, and no npm install. `node tools/quant-sweep.mjs --synthetic` is a self-test on fake candles (results meaningless). Run it, then apply the winning settings as defaults only if they hold on TEST and on a later re-run.
